...
- 必要権限:
  ...
  - Moderate Members（10 分間に 3 回のスパム削除判定または類似画像の管理者レビュー判定を受けたユーザーを 10 分間タイムアウトするため）
...
- 同一サーバーで同じユーザーが 10 分間に 3 件以上の `delete` 判定（スパム削除）または `review` 判定（類似画像の管理者確認）を受けると、Bot はそのユーザーを Discord のタイムアウト機能で 10 分間タイムアウトします。
- 複数画像を同じメッセージに添付した場合は 1 件として数えます。`allow` 判定は数えません。
- Bot に **Moderate Members** 権限があり、Bot のロールが対象ユーザーのロールより上位である必要があります。権限またはロール階層の不足時は、検知処理を継続しつつタイムアウトだけをログに警告します。
- Discord の通常チャンネルメッセージは特定ユーザーだけに表示できないため、タイムアウトに成功した場合は対象ユーザーだけが読めるDMで「スパム検知されたため、一時的なタイムアウト処置が実行されました。誤検知の場合はサーバー管理者にお問い合わせください」と通知します。DMを拒否している場合でもタイムアウト処置は継続し、通知失敗はログに記録します。
