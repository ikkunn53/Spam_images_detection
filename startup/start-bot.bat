...
if exist "%PROJECT_ROOT%\ai-service\.env" (
  for /F "usebackq tokens=1,* delims==" %%A in ("%PROJECT_ROOT%\ai-service\.env") do (
...
start "Image Spam AI Service" /D "%PROJECT_ROOT%\ai-service" cmd /k ".venv\Scripts\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port %AI_SERVICE_PORT%"

echo Waiting for AI Service health check...
set "AI_READY=0"
for /L %%I in (1,1,30) do (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:%AI_SERVICE_PORT%/health' -TimeoutSec 2; if ($r.StatusCode -eq 200) { exit 0 } } catch { exit 1 }" >nul 2>nul
  if not errorlevel 1 (
    set "AI_READY=1"
    goto :ai_ready
  )
