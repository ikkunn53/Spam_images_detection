...
export const configureLogChannelCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'set') {
      ...
      await interaction.reply({ content: `画像スパム検知ログの送信先を <#${channel.id}> に設定しました。`, flags: MessageFlags.Ephemeral });
      return;
    }

    if (subcommand === 'clear') {
      guildSettings.setLogChannel(interaction.guildId, null);
      await interaction.reply({ content: '画像スパム検知ログの送信先設定を解除しました。', flags: MessageFlags.Ephemeral });
      return;
    }

    const settings = guildSettings.get(interaction.guildId);
    await interaction.reply({
      content: settings.log_channel_id ? `現在の画像スパム検知ログ送信先は <#${settings.log_channel_id}> です。` : '画像スパム検知ログの送信先は未設定です。',
